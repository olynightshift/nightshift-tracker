...
import { Calendar, Package, Shirt, Receipt as ReceiptIcon, Download, Plus, Trash2, Search, Home, Camera, ChevronLeft, ChevronRight, Gift, X, Pencil, Check } from 'lucide-react';
...
function OutreachTab({ entries = [], setEntries = () => {} }) {
  ...
  const isEditingExisting = !!entryMap[formDate];
  ...
  function deleteEntry(date) {
    ...
    if (date === formDate) { setFormNightshift(false); setFormCoffee(false); }
  }
  function exportCSV() {
  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="bg-white rounded-lg shadow p-4">
        ...
        {isEditingExisting && <div className="text-xs text-amber-600 bg-amber-50 border border-amber-200 rounded px-2 py-1 mb-3 inline-block">Editing existing entry for {formDate}</div>}
        <div className="flex flex-wrap items-end gap-4">
          ...
          <button onClick={saveEntry} className="bg-blue-600 text-white px-4 py-1.5 rounded hover:bg-blue-700 flex items-center gap-1">
            {isEditingExisting ? <Check size={16} /> : <Plus size={16} />}
            {isEditingExisting ? 'Update Day' : 'Save Day'}
          </button>
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {weekDays.map(d => {
                ...
                return (
                  <tr key={iso} className={`border-b hover:bg-gray-50 cursor-pointer ${isSelected ? 'bg-yellow-50' : ''}`} onClick={() => loadDay(iso)}>
                    <td className="py-1.5">{DAYS[d.getDay()]}</td>
                    <td>{iso} {isSelected && <Pencil size={12} className="inline text-amber-500 ml-1" />}</td>
                    <td>{e?.nightshift ? '✅' : '—'}</td>
                    <td>{e?.coffee ? '✅' : '—'}</td>
        ...
        <div className="bg-white rounded-lg shadow p-4">
          ...
          <div className="grid grid-cols-7 gap-1">
            {monthGridDays.map((d, i) => {
              ...
              const isSelected = iso === formDate;
              return (
                <div key={i} onClick={() => loadDay(iso)} className={`border rounded p-1 h-16 text-xs cursor-pointer ${isSelected ? 'ring-2 ring-amber-400 bg-yellow-50' : inMonth ? 'bg-white' : 'bg-gray-50 text-gray-300'} hover:ring-2 ring-blue-300`}>
                  <div>{d.getDate()}</div>
...
function BlanketTab({ entries = [], setEntries = () => {} }) {
  ...
  function deleteEntry(id) { setEntries(prev => prev.filter(e => e.id !== id)); if (id === editingId) setEditingId(null); }
  function startEdit(entry) { setEditingId(entry.id); setEditDraft({ date: entry.date, blankets: entry.blankets, coats: entry.coats, pounds: entry.pounds }); }
  function cancelEdit() { setEditingId(null); setEditDraft({}); }
  function saveEdit() {
    setEntries(prev => prev.map(e => e.id === editingId ? { id: editingId, date: editDraft.date, blankets: Number(editDraft.blankets) || 0, coats: Number(editDraft.coats) || 0, pounds: Number(editDraft.pounds) || 0 } : e));
    setEditingId(null);
  }
  function exportCSV() {
  ...
  function renderRow(e) {
    if (editingId === e.id) {
      return (
        <tr key={e.id} className="border-b bg-yellow-50">
          <td className="py-1.5"><input type="date" value={editDraft.date} onChange={ev => setEditDraft(d => ({ ...d, date: ev.target.value }))} className="border rounded px-1 py-0.5 text-sm w-32" /></td>
          <td><input type="number" min="0" value={editDraft.blankets} onChange={ev => setEditDraft(d => ({ ...d, blankets: ev.target.value }))} className="border rounded px-1 py-0.5 text-sm w-16" /></td>
          <td><input type="number" min="0" value={editDraft.coats} onChange={ev => setEditDraft(d => ({ ...d, coats: ev.target.value }))} className="border rounded px-1 py-0.5 text-sm w-16" /></td>
          <td><input type="number" min="0" step="0.1" value={editDraft.pounds} onChange={ev => setEditDraft(d => ({ ...d, pounds: ev.target.value }))} className="border rounded px-1 py-0.5 text-sm w-20" /></td>
          <td className="flex gap-2 py-1.5">
            <button onClick={saveEdit}><Check size={14} className="text-green-600" /></button>
            <button onClick={cancelEdit}><X size={14} className="text-gray-500" /></button>
          </td>
        </tr>
      );
    }
    return (
      <tr key={e.id} className="border-b">
        ...
        <td className="flex gap-2 py-1.5">
          <button onClick={() => startEdit(e)}><Pencil size={14} className="text-blue-500" /></button>
          <button onClick={() => deleteEntry(e.id)}><Trash2 size={14} className="text-red-500" /></button>
        </td>
      </tr>
    );
  }
      ...
      {view === 'week' && (
        <div className="bg-white rounded-lg shadow p-4">
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {weekEntries.sort((a, b) => a.date.localeCompare(b.date)).map(renderRow)}
      ...
      {view === 'month' && (
        <div className="bg-white rounded-lg shadow p-4">
          ...
          <table className="w-full text-sm mt-4">
            <thead><tr className="text-left border-b"><th className="py-1">Date</th><th>Blankets</th><th>Coats/Hoodies</th><th>Pounds</th><th></th></tr></thead>
            <tbody>
              {monthEntries.sort((a, b) => a.date.localeCompare(b.date)).map(renderRow)}
...
function DonationTab({ entries = [], setEntries = () => {}, itemTypes = [], setItemTypes = () => {} }) {
  ...
  const [yearSelected, setYearSelected] = useState(new Date().getFullYear());
  const [editingId, setEditingId] = useState(null);
  const [editDraft, setEditDraft] = useState({ date: todayISO, quantities: {} });
  ...
  function startEdit(entry) {
    ...
    itemTypes.forEach(t => { initQuantities[t] = entry.quantities[t] ?? 0; });
    setEditingId(entry.id);
    setEditDraft({ date: entry.date, quantities: initQuantities });
  }
  function cancelEdit() { setEditingId(null); }
  function updateEditQuantity(type, value) { setEditDraft(d => ({ ...d, quantities: { ...d.quantities, [type]: value } })); }
  function saveEdit() {
    setEntries(prev => prev.map(e => {
      if (e.id !== editingId) return e;
      const quantities = { ...e.quantities };
      itemTypes.forEach(t => { quantities[t] = Number(editDraft.quantities[t]) || 0; });
      return { ...e, date: editDraft.date, quantities };
    }));
    setEditingId(null);
  }
  function exportCSV() {
    ...
    if (editingId === e.id) {
      return (
        <tr key={e.id} className="border-b bg-yellow-50">
          <td className="py-1.5"><input type="date" value={editDraft.date} onChange={ev => setEditDraft(d => ({ ...d, date: ev.target.value }))} className="border rounded px-1 py-0.5 text-sm w-32" /></td>
          {itemTypes.map(t => (
            <td key={t}><input type="number" min="0" value={editDraft.quantities[t] ?? ''} onChange={ev => updateEditQuantity(t, ev.target.value)} className="border rounded px-1 py-0.5 text-sm w-16" /></td>
          ))}
          <td className="flex gap-2 py-1.5">
            <button onClick={saveEdit}><Check size={14} className="text-green-600" /></button>
            <button onClick={cancelEdit}><X size={14} className="text-gray-500" /></button>
...
function ClosetTab({ entries = [], setEntries = () => {} }) {
  ...
  function deleteEntry(id) { setEntries(prev => prev.filter(e => e.id !== id)); if (id === editingId) setEditingId(null); }
  function startEdit(entry) { setEditingId(entry.id); setEditDraft({ date: entry.date, people: entry.people }); }
  function cancelEdit() { setEditingId(null); }
  function saveEdit() {
    setEntries(prev => prev.map(e => e.id === editingId ? { id: editingId, date: editDraft.date, people: Number(editDraft.people) || 0 } : e));
    setEditingId(null);
  }
  function exportCSV() {
  ...
  function renderRow(e) {
    if (editingId === e.id) {
      return (
        <tr key={e.id} className="border-b bg-yellow-50">
          <td className="py-1.5"><input type="date" value={editDraft.date} onChange={ev => setEditDraft(d => ({ ...d, date: ev.target.value }))} className="border rounded px-1 py-0.5 text-sm w-32" /></td>
          <td><input type="number" min="0" value={editDraft.people} onChange={ev => setEditDraft(d => ({ ...d, people: ev.target.value }))} className="border rounded px-1 py-0.5 text-sm w-20" /></td>
          <td className="flex gap-2 py-1.5">
            <button onClick={saveEdit}><Check size={14} className="text-green-600" /></button>
            <button onClick={cancelEdit}><X size={14} className="text-gray-500" /></button>
          </td>
...
function ReceiptsTab({ receipts = [], setReceipts = () => {} }) {
  ...
  const [editingId, setEditingId] = useState(null);
  const [editDraft, setEditDraft] = useState({ date: todayISO, store: '', items: [], image: null });
  ...
  function deleteReceipt(id) { setReceipts(prev => prev.filter(r => r.id !== id)); if (id === editingId) setEditingId(null); }
  function exportCSV() {
    const rows = [];
  ...
  function startEdit(r) {
    setEditingId(r.id);
    setEditDraft({ date: r.date, store: r.store, items: r.items.map(it => ({ ...it, cost: String(it.cost) })), image: r.image });
  }
  function cancelEdit() { setEditingId(null); }
  function updateEditItem(i, field, value) { setEditDraft(d => ({ ...d, items: d.items.map((it, idx) => idx === i ? { ...it, [field]: value } : it) })); }
  function addEditItemRow() { setEditDraft(d => ({ ...d, items: [...d.items, { name: '', cost: '', category: CATEGORIES[0] }] })); }
  function removeEditItemRow(i) { setEditDraft(d => ({ ...d, items: d.items.filter((_, idx) => idx !== i) })); }
  function handleEditImage(e) {
    const file = e.target.files[0];
    if (!file) return;
    const reader = new FileReader();
    reader.onload = () => setEditDraft(d => ({ ...d, image: reader.result }));
    reader.readAsDataURL(file);
  }
  function saveEdit() {
    if (!editDraft.store || editDraft.items.every(it => !it.name && !it.cost)) { alert('Please enter a store and at least one item.'); return; }
    setReceipts(prev => prev.map(r => r.id === editingId ? {
      ...r,
      date: editDraft.date,
      store: editDraft.store,
      items: editDraft.items.filter(it => it.name || it.cost).map(it => ({ ...it, cost: Number(it.cost) || 0 })),
      ...
      {view === 'list' && (
        <div className="flex flex-col gap-3">
          ...
          {filtered.slice().sort((a, b) => b.date.localeCompare(a.date)).map(r => {
            if (editingId === r.id) {
              return (
                <div key={r.id} className="bg-yellow-50 border border-yellow-300 rounded-lg shadow p-4">
                  <div className="flex flex-wrap items-end gap-4 mb-3">
                    <div>
                      <label className="block text-sm text-gray-600 mb-1">Date</label>
                      <input type="date" value={editDraft.date} onChange={e => setEditDraft(d => ({ ...d, date: e.target.value }))} className="border rounded px-2 py-1" />
                    </div>
                    <div>
                      <label className="block text-sm text-gray-600 mb-1">Store</label>
                      <input type="text" value={editDraft.store} onChange={e => setEditDraft(d => ({ ...d, store: e.target.value }))} className="border rounded px-2 py-1" />
                    </div>
                    <div>
                      <label className="block text-sm text-gray-600 mb-1 flex items-center gap-1"><Camera size={14} /> Replace Photo</label>
                      <input type="file" accept="image/*" onChange={handleEditImage} className="text-sm" />
                    </div>
                    {editDraft.image && <img src={editDraft.image} alt="receipt" className="h-16 rounded border" />}
                  </div>
                  <div className="flex flex-col gap-2 mb-3">
                    {editDraft.items.map((it, i) => (
                      <div key={i} className="flex flex-wrap items-center gap-2">
                        <input type="text" placeholder="Item name" value={it.name} onChange={e => updateEditItem(i, 'name', e.target.value)} className="border rounded px-2 py-1 flex-1" />
                        <input type="number" step="0.01" placeholder="Cost" value={it.cost} onChange={e => updateEditItem(i, 'cost', e.target.value)} className="border rounded px-2 py-1 w-24" />
                        <select value={it.category} onChange={e => updateEditItem(i, 'category', e.target.value)} className="border rounded px-2 py-1">
                          {CATEGORIES.map(c => <option key={c} value={c}>{c}</option>)}
                        </select>
                        <button onClick={() => removeEditItemRow(i)}><Trash2 size={16} className="text-red-500" /></button>
                      </div>
                    ))}
                    <button onClick={addEditItemRow} className="text-sm text-blue-600 flex items-center gap-1 w-fit"><Plus size={14} />Add Item</button>
                  </div>
                  <div className="flex items-center justify-between">
                    <div className="text-sm font-medium">Total: ${editDraft.items.reduce((s, it) => s + (Number(it.cost) || 0), 0).toFixed(2)}</div>
                    <div className="flex gap-2">
                      <button onClick={saveEdit} className="bg-green-600 text-white px-4 py-1.5 rounded hover:bg-green-700 flex items-center gap-1"><Check size={16} />Save Changes</button>
                      <button onClick={cancelEdit} className="bg-gray-200 text-gray-700 px-4 py-1.5 rounded hover:bg-gray-300 flex items-center gap-1"><X size={16} />Cancel</button>
                    </div>
                  </div>
                </div>
              );
              ...
              <div key={r.id} className="bg-white rounded-lg shadow p-4">
                <div className="flex justify-between items-start">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <button onClick={() => startEdit(r)}><Pencil size={16} className="text-blue-500" /></button>
